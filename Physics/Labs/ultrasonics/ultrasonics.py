...
attenuation_data = pd.read_csv(filepath_or_buffer='https://docs.google.com/spreadsheets/'\
                                                'd/e/2PACX-1vSQkYXTg7kJ8iv9UbEYUtghgHdpPd'\
                                                'NBxVMFQ-NMDZGb8XzZojzFp-kzFKE1kdziMX-_IlRR'\
                                                'ou1X3A8v/pub?gid=1776301516&single=true&output=csv')
...
def speed_of_sound_calculations(data: pd.DataFrame) -> float:
    ...
    nodes = np.array(ultrasonics_data.loc[:28, 'n']).astype(float)
    freq = np.array(ultrasonics_data.loc[:28, 'measured_f(khz)']).astype(float) * 1e3
    errors = np.array(ultrasonics_data.loc[:28, 'error_f']).astype(float)
    ...
    axes.text(15, 12, f"m = ({gradient:.2f} +/- {gradient_error:.2f}) " + r'$s^{-1}$'\
                        f"\nc = ({sound_speed:.2f} +/- {sound_error:.2f}) " + r'$ms^{-1}$')
    ...
    # plt.savefig("cw_ultrasonics.png", dpi=300)
    plt.show()
...
def plot_response_curve(data: pd.DataFrame, speed, speed_error) -> None:
    ...
    freq2 = np.array(attenuation_data.loc[:, 'freq(khz)']).astype(float)
    amp_data = np.array(attenuation_data.loc[:, 'amplitude(mV)'])
    errors_amp = np.array(attenuation_data.loc[:, 'error'])
