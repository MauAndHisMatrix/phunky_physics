...
def plot_response_curve(data: pd.DataFrame, speed, speed_error) -> None:
    ...
    # axes2.text(3.933, 950, f"Q = {quality:.2f}" + r" $\pm$ " + f"{q_error:.2f}\n"\
    #                         r"$\alpha$ = " + f"{alpha:.2e}" + r" $\pm$ " + f"{alpha_error:.2e}\n"\
    #                             r"$\chi^{2}$ = " + f"{chi:.2f}\n"\
    #                                 r"$\chi^{2}_{red}$ = " + f"{red_chi:.2f}", fontsize=9)

    # axes2.text(11.922, 120, f"Q = {quality:.2f}" + r" $\pm$ " + f"{q_error:.2f}\n"\
    #                         r"$\alpha$ = " + f"{alpha:.2e}" + r" $\pm$ " + f"{alpha_error:.2e}\n"\
    #                             r"$\chi^{2}$ = " + f"{chi:.2f}\n"\
    #                                 r"$\chi^{2}_{red}$ = " + f"{red_chi:.2f}", fontsize=9)

    axes2.text(39.93, 110, f"Q = {quality:.2f}" + r" $\pm$ " + f"{q_error:.2f}\n"\
                            r"$\alpha$ = " + f"{alpha:.2e}" + r" $\pm$ " + f"{alpha_error:.2e}\n"\
                                r"$\chi^{2}$ = " + f"{chi:.2f}\n"\
                                    r"$\chi^{2}_{red}$ = " + f"{red_chi:.2f}", fontsize=9)

    axes2.set_title("Resonant Response Curve for n=20", fontsize=16, color="black")
    axes2.set_xlabel("Frequency (kHz)", fontsize=12)
    axes2.set_ylabel("Amplitude (mV)", fontsize=12)

    # axes2.set_ylim([550, 1090])
    # axes2.set_ylim([60, 151])
    axes2.set_ylim([80, 125])


    plt.legend()
    plt.savefig("n20_response_ultrasonics.png", dpi=300)
    plt.show()
