...
def current_func(current_data):
    ...
    axes.errorbar(data[:, 0], data[:, 1], yerr=data[:, 2], fmt='o', markersize=4, color='red')
    ...
    axes.text(-18, 160, r"$\chi^{2}$ = " + f"{chi:.3f}\n" + r"$\chi_{red}^{2}$ = " + f"{red_chi:.3f}",
              fontsize=16)

    axes.set_title(r"V$_{g}$ vs V$_{a}$ at fixed I$_{a}$", fontsize=20, color="black")
    axes.set_xlabel(r"V$_{g}$ (V)", fontsize=16)
    axes.set_ylabel(r"V$_{a}$ (V)", fontsize=16)
    axes.legend(loc='upper right', fontsize=16)

    # Residuals
    axes2 = fig.add_subplot(212)

    residuals = data[:, 1] - straight_line(data[:, 0], gradient, intercept)
    axes2.errorbar(data[:, 0], residuals, yerr=data[:, 2], fmt='o', markersize=6, color='red')

    axes2.plot(data[:, 0], [0]*data.shape[0], color='blue', linewidth=2)

    axes2.set_title("Residuals", fontsize=20, color="black")
    axes2.set_xlabel(r"V$_{g}$ (V)", fontsize=16)
    axes2.set_ylabel(r"V$_{a}^{fit}$ - V$_{a}^{obs}$ (V)", fontsize=16)

    plt.tight_layout()
    plt.savefig("v_g_v_a_i_700_res.png", dpi=900)
    # plt.show()
