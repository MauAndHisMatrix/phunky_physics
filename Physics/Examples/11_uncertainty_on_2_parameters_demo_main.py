...
FILE_NAME = r"C:\Users\Marawan\Desktop\Python\Physics\Examples\polynomial_data.csv"
...
def plot_raw_data(data):
    """
    Plots errorbar plot of data.
    Args:
        data array of floats, [float, float, float]
    Returns:
        None
    """
...
def plot_fitted_data(data, fit):
    """
    Plots errorbar of data and line of best fit.
    Args:
        data: array of floats, [float, float, float]
        fit: array of floats
    Returns:
    ...
    fitted_data_plot.set_title(r'Fitted data. a = {0:3.2f}, b = {1:3.2f}.'.
                               ...
                               format(fit[0] / (len(data) - 2)))
    fitted_data_plot.set_xlabel('x')
    fitted_data_plot.set_ylabel('f(x)')

    fitted_data_plot.plot(data[:, 0], function(data[:, 0], fit[1][0],
                                               fit[1][1]))
...
def main():
    ...
    plot_fitted_data(data, fit)
    # Contour plot of parameters, this could also be written as a function
    # Note: these are hardcoded, not best practice.
    a_values = np.linspace(fit[1][0] - 0.05,
                           fit[1][0] + 0.05, 500)
    b_values = np.linspace(fit[1][1] - 0.3,
                           fit[1][1] + 0.3, 500)
    ...
    parameters_contour_plot.scatter(fit[1][0], fit[1][1],
                                    label='Minimum')
    ...
    # Ideally these numbers would be defined elsewhere so they are easy to
    # ammend without having to ammend several things.
    chi_squared_levels = (fit[0] + 2.30, fit[0] + 5.99, fit[0] + 9.21)
