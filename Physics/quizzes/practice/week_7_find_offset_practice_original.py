...
input_file = open('week_8_find_offset_data_practice.csv', 'r')
...
for line in input_file:
    ...
    split_up = line.split(',')
    if not is_float(split_up[0]) and is_float(split_up[1]):
...
while difference > TOLERANCE:
    ...
    comparison_0 = np.sum((data[:, 0] - polynomial(data[:, 0], offset))**2)
