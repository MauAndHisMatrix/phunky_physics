...
def remove_outliers(data: np.ndarray,
    ...
    """
    initial_func = activity_79_rb_function(data[:, 0],
...
def minimise_chi_squared(data: np.ndarray,
                         fitting_function: Callable) -> tuple:
    """
    ...
    Returns:
        ...
        chi-squared result.
    """
    rb_start_value, sr_start_value = [0.0005, 0.005]
    # rb_start_value, sr_start_value = [1, 1]
    results = \
        fmin(lambda decay_consts: calculate_chi_squared(data,
                                            fitting_function,
                                            decay_consts[0], decay_consts[1]),
                                    (rb_start_value, sr_start_value),
...
def plot_data(data: np.ndarray,
    ...
    axes.set_ylabel("Activity levels (Bq)", fontsize=12)
...
def main():
    ...
    data = remove_outliers(data, minimised_initial_consts)
